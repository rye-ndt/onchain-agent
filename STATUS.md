...
**Sign-resolution UX — 2026-04-27.** All three CLIs (`telegramCli`/`httpCli`/`workerCli`) share `helpers/notifyResolved.ts` for the single Telegram message that fires when a signing request resolves. The use case propagates resolution via `SigningResolutionEvent` (`{ chatId, userId, txHash?, rejected, errorCode?, errorMessage?, data?, to? }`).

- **Success.** Decodes the userOp calldata with `helpers/decodeErc20Transfer.ts` (selector-scan for `transfer(address,uint256)`; matches Kernel `executeBatch` layout). When matched, posts "✅ Transaction confirmed. You sent *X TOKEN* to \`0xRECP…IENT\`" with an inline-keyboard "🔍 View on explorer" URL button. Explorer URL comes from `getExplorerTxUrl(chainId, txHash)` in `chainConfig.ts`, which reads viem's `Chain.blockExplorers.default.url` — never hardcoded. Non-transfer calldata (swap, yield deposit) → generic "✅ Transaction submitted." with the same explorer button. Markdown-send falls back to plain on parse failure.
- **Insufficient-balance recovery.** When the FE's `interpretSignError` classifies a sendTransaction failure (`SignErrorCode` enum), `SignHandler` posts `{ rejected: true, errorCode, errorMessage }` to `POST /response`. On `errorCode === 'insufficient_token_balance'` and the failed token == chain USDC, `notifyResolved` sends a Telegram nudge with an inline keyboard reusing `BuyCapability`'s existing `buy:y:<amount>` / `buy:n:<amount>` callbacks → user lands directly in the /buy confirm step. Non-USDC or decode failure → plain friendly-message reply (onramp is USDC-only).
